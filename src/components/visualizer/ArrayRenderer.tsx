  ...
  return (
    <div className="w-full h-full flex flex-col rounded-2xl relative overflow-hidden">
      ...
      <div className={clsx("px-5 py-3 flex items-center gap-3 border-b shrink-0", op.bg, op.border)}>
        ...
        <span className="text-text-secondary text-sm flex-1 truncate">{msg}</span>
      </div>
        ...
        <div className="flex items-end gap-2 sm:gap-3">
          <AnimatePresence mode="popLayout">
            {data.map((val: number, index: number) => {
              ...
              let cardStyle = 'bg-surface/80 border-surface-border/50 text-text-primary';
              let shadow = '';
              let label = '';
              let scale = 1;
                ...
                >
                  ...
                  {isActive && label && (
                    <motion.div
                      ...
                      className={clsx("text-lg font-bold", isActive && operationType === OperationType.OVERWRITE ? 'text-orange-400' : 'text-text-primary/80')}
                    >
                      {label}
                    </motion.div>
                  )}
      ...
      {/* Bottom: Color legend */}
      <div className="px-5 py-2.5 border-t border-surface/50 flex items-center justify-center gap-6 text-[11px] font-medium text-text-secondary shrink-0 bg-background/30">
